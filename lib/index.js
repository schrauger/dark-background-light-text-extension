...
function update_options() {
    ...
    update_global_sheet(!(simple_prefs.prefs['scheduleEnabled']));
}
...
function get_settings_init_params() {
    return {
        ...
        scheduleEnabled: simple_prefs.prefs['scheduleEnabled'],
        methods: methods,
...
preferences.forEach(pref => {
    simple_prefs.on(pref.name, key => {
        ...
        if (['default_method', 'enabled', 'scheduleEnabled', 'configured_pages'].indexOf(key) >= 0) {
            if (key === 'configured_pages')
                return;
            if ((key === 'enabled') || (key === 'scheduleEnabled'))
                update_global_sheet(!(simple_prefs.prefs['enabled']));
            gppmm.broadcastAsyncMessage('update_applied_methods');
        }
        else
...
ui.on('panel-port-schedule-enabled-change', scheduleEnabled => {
    ui.hide();
    simple_prefs.prefs['scheduleEnabled'] = scheduleEnabled;
});
